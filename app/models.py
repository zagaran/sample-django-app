...
class SampleObject(TimestampedModel):
    ...
    # In order for the `DirectUploadFileField` to work properly on `ModelForm` instances, make sure that ForeignKey
    # relationships between objects and attachments are pointing from the object to the attachment, not the other way
    # around.
